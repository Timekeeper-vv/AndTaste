...
public class CommercialProductizationController {
    ...
    public Map<String, Object> consumerRequests(
        ...
        out.put("quoteRequests", jdbc.queryForList("SELECT r.id,r.request_no requestNo,r.asset_id assetId,r.request_type requestType,r.quantity,r.purpose,r.status,r.quoted_unit_price quotedUnitPrice,r.quoted_total_price quotedTotalPrice,r.quoted_lead_time quotedLeadTime,r.operator_comment operatorComment,CASE WHEN r.request_type='sample' AND r.status='accepted' AND r.sample_payment_status='not_required' THEN 'unpaid' ELSE r.sample_payment_status END samplePaymentStatus,r.sample_payment_order_no samplePaymentOrderNo,r.sample_paid_at samplePaidAt,p.template_code templateCode,p.product_name productName FROM creative_quote_request r JOIN creative_product_template p ON p.id=r.product_template_id WHERE r.user_id=? ORDER BY r.id DESC LIMIT 100", userId));
