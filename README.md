...
- `Google Imagen 4`：默认排在首位，通过 Replicate `google/imagen-4` 生成，支持 `1K/2K`、`1:1/16:9/9:16/4:3/3:4` 和 `png/jpg`，结果自动保存到系统资产库。
  ...
  - 优化完成后同时返回“产品使用说明”，说明产品定位、适用场景、使用方法、保养/储存方式和安全注意事项。
...
项目不再自动创建任何默认用户名或演示密码。首次部署时，如数据库中还没有管理员，可在服务器 `.env` 中临时填写 `BOOTSTRAP_ADMIN_*` 并将 `BOOTSTRAP_ADMIN_ENABLED=true`；服务启动成功、确认可以登录后，立即改回 `false` 并重启。管理员密码至少 12 个字符，且所有账号密码都应使用高强度随机值。
...
迁移不会删除历史账号；请由管理员人工盘点并停用演示账号、重置所有旧密码。JWT 会在账号删除、改名或降权后立即失效。
...
- 域名上线后启用 HTTPS
- `CORS_ALLOWED_ORIGINS` 只填写实际网页前端 HTTPS 域名，禁止使用 `*`
- 创作资产目录放在网页静态目录之外，并通过鉴权接口访问
