...
- `墨刀 AI 设计`：作为外部设计工作区接入。系统会使用 Qwen3 优化 Prompt，点击生成时打开配置的墨刀 AI 设计链接，并尝试复制 Prompt；墨刀产出的图片需人工从墨刀下载后上传到资产库。
...
注意：墨刀令牌必须放在 `shixun/application-local.properties` 或环境变量中，不能提交到仓库。当前墨刀链接不是公开的文生图回传 API，因此系统不会编造“自动生成并入库”的能力。
