...
注意：墨刀令牌必须放在 `shixun/application-local.properties` 或环境变量中，不能提交到仓库。若墨刀返回 `INVALID_TOKEN`，需要到墨刀头像 → 令牌设置重新创建 MCP/API 令牌；若返回 `insufficient_points`，说明 MCP 已连通但墨刀账号积分不足。
