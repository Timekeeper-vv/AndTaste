...
| 流程类型 | 适用场景 | 说明 |
|---|---|---|
| 标准审批 | 普通连锁、市场、项目、人事、考勤事项 | 一级主管审批，支持或签 |
| 两级审批 | 财务、生产等关键事项 | 主管审批后进入终审确认 |
| 会签审批 | 重大或跨部门事项 | 多人会签后进入终审 |
...
- AI 查询采用 Text-to-API：先把自然语言转换为结构化查询参数，再由后端查询 `supply_chain_sample_work_order`，最后基于真实结果回答；禁止编造项目、产品、负责人、数量和日期
...
| `/api/workflows/applications/{id}/transfer` | `POST` | 转交给指定处理人 |
