...
- `墨刀 AI 设计`：后端调用墨刀 MCP `gen_html` 生成设计稿 HTML，再使用本机 Chrome headless 渲染成 PNG，自动保存到系统资产库并回传到 2D 页面展示。
...
modao.api.base-url=https://modao.cc
modao.chrome.path=/Applications/Google Chrome.app/Contents/MacOS/Google Chrome
```

注意：墨刀令牌必须放在 `shixun/application-local.properties` 或环境变量中，不能提交到仓库。若墨刀返回 `INVALID_TOKEN`，需要到墨刀头像 → 令牌设置重新创建 MCP/API 令牌。
