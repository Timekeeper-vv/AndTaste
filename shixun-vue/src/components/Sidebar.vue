...
const allMenus: MenuGroup[] = [
  { group: '总览', items: [
    ...
    { key: 'consumerProductionReview', label: '3D建模审核', parentKey: 'approvalCenter', roles: SUPER_ADMIN_ROLES, icon: `<svg></svg>` },
    { key: 'orderManagement', label: '订单管理', parentKey: 'approvalCenter', roles: SUPER_ADMIN_ROLES, icon: `<svg></svg>` },
    { key: 'commercialProductization', label: '商品化与代销审核', parentKey: 'approvalCenter', roles: MANAGER_ROLES, icon: `<svg></svg>` },
    { key: 'professionalGuidance', label: '专业指导', parentKey: 'approvalCenter', roles: MANAGER_ROLES, icon: `<svg></svg>` },
    { key: 'customerService', label: 'C端客服会话', roles: ALL_ROLES, icon: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 15a4 4 0 0 1-4 4H8l-4 3v-7a4 4 0 0 1-1-3V7a4 4 0 0 1 4-4h9a4 4 0 0 1 4 4v8Z"/><path d="M8 10h.01M12 10h.01M16 10h.01"/></svg>` },
  ...
  { group: '系统', items: [
    ...
    { key: 'historicalSales', label: '历史销售数据', roles: SUPER_ADMIN_ROLES, icon: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 19V5"/><path d="M4 19h16"/><path d="M7 15l3-4 3 2 5-7"/></svg>` },
    ...
    <nav class="sidebar-nav">
      <template v-for="menu in menus" :key="menu.group">
        ...
        <button
          ...
          :key="item.key"
          class="nav-item"
          :class="{ active: currentPage === item.key, parent: ['approvalCenter','studio','scaleUp','warehouseLogistics','chain','marketDemand','projectDemand','hrManagement','attendanceManagement','finance'].includes(item.key), child: !!item.parentKey }"
        ...
        >
          ...
          <transition name="fade">
            <span v-if="!collapsed" class="nav-content">
              ...
              <span v-if="['approvalCenter','studio','scaleUp','warehouseLogistics','chain','marketDemand','projectDemand','hrManagement','attendanceManagement','finance'].includes(item.key)" class="parent-arrow">⌄</span>
            </span>
          </transition>
          <transition name="fade">
