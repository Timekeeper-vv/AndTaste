  ...
  <div class="review-page">
    ...
    <section v-if="isMultiviewMode && visibleMultiViewBundles.length" class="multiview-review-panel">
      ...
      <div class="multiview-review-grid">
        <article v-for="bundle in visibleMultiViewBundles" :key="bundle.id" class="multiview-review-card">
          <div class="bundle-review-top"><div><strong>{{ bundle.productName || '三视图文创作品' }}</strong><small class="product-identity-inline">产品号：{{ bundle.productNo || '未关联产品号' }}</small><small>作品包：{{ bundle.bundleNo || `#${bundle.id}` }} · 用户 {{ bundle.userId || '-' }} · {{ bundle.username || '-' }}</small></div><span class="status-pill" :class="statusClass(bundle.status)">{{ bundle.status === 'approved' && bundle.sampleRequest?.samplePaymentStatus === 'paid' ? '已支付，生产中' : bundle.status === 'approved' && bundle.sampleRequest ? '待用户支付打样费' : statusText[bundle.status || 'review'] || bundle.status }}</span></div>
    ...
    <section class="work-grid" v-if="isStandardMode && visibleWorks.length">
      <article v-for="w in visibleWorks" :key="w.id" class="work-card">
        ...
        <div class="work-body">
          <div class="product-identity"><span>产品号</span><b :class="{ missing: !w.productNo }">{{ w.productNo || '未关联产品号' }}</b></div>
...
<style scoped>
.product-identity{display:flex;align-items:center;gap:7px;margin:0 0 10px;padding-bottom:10px;border-bottom:1px solid #edf2ed}.product-identity span{color:#7f8f84;font-size:11px;font-weight:800}.product-identity b{display:inline-block;padding:5px 8px;border:1px solid #c9dfce;border-radius:6px;background:#eef8f0;color:#2f6844;font-size:12px;letter-spacing:.02em}.product-identity b.missing,.product-no-cell strong.missing{border-color:#e6dfca;background:#fcfaf2;color:#8b8064;font-weight:600}.product-identity-inline{color:#2f6844!important;font-weight:900}.product-no-col{width:150px}.product-no-cell{background:#fbfefb}.product-no-cell strong{display:inline-block;padding:5px 8px;border:1px solid #c9dfce;border-radius:6px;background:#eef8f0;color:#2f6844;font-size:12px;letter-spacing:.02em}
</style>
