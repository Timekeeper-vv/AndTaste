...
const props = defineProps<{ modelUrl: string; modelName?: string; externalSaving?: boolean }>()
const emit = defineEmits<{ loaded: []; error: [message: string]; saveVariant: [payload: { blob: Blob; materialLabel: string }] }>()

type PresetKey = 'original' | 'ceramic' | 'gold' | 'metal' | 'wood' | 'stainedPine' | 'ppc' | 'abs' | 'pvc' | 'tangjiao' | 'softVinyl' | 'plush' | 'shortPlush' | 'superSoftPlush' | 'acrylic' | 'resin' | 'paper'
type TextureSet = { baseColor: string; bump?: string; repeat?: number; bumpScale?: number }
type MaterialFamily = 'standard' | 'ppc' | 'plush'
type MaterialPreset = { key: PresetKey; label: string; hint: string; color: number; metalness: number; roughness: number; family?: MaterialFamily; clearcoat?: number; transmission?: number; opacity?: number; sheen?: number; sheenRoughness?: number; textures?: TextureSet }
const presets: MaterialPreset[] = [
  ...
  { key: 'ppc', label: 'PPC 高精硬塑', hint: '注塑微纹 · 精密分件感', color: 0xd5d9d7, metalness: 0, roughness: .34, family: 'ppc', clearcoat: .08 },
  { key: 'abs', label: 'ABS 工程硬塑', hint: '细腻硬壳 · 稳定量产感', color: 0xe1e6e2, metalness: 0, roughness: .3, family: 'ppc', clearcoat: .12 },
  { key: 'pvc', label: 'PVC 潮玩', hint: '半哑乙烯 · 收藏玩具感', color: 0xffa083, metalness: 0, roughness: .37, clearcoat: .16 },
  { key: 'tangjiao', label: '搪胶（糖胶）', hint: '中空圆润 · 软触潮玩', color: 0xe99b82, metalness: 0, roughness: .48, clearcoat: .08, sheen: .28, sheenRoughness: .72 },
  { key: 'softVinyl', label: '软胶', hint: '柔韧包胶 · 低反射表面', color: 0xb99e90, metalness: 0, roughness: .58, clearcoat: .03, sheen: .14, sheenRoughness: .86 },
  { key: 'plush', label: '全毛绒', hint: '蓬松纤维 · 可自选颜色', color: 0xe9ad83, metalness: 0, roughness: .93, family: 'plush', sheen: 1, sheenRoughness: .58 },
  { key: 'shortPlush', label: '短毛绒', hint: '细密短绒 · 轮廓更清晰', color: 0xd9b38e, metalness: 0, roughness: .88, family: 'plush', sheen: .82, sheenRoughness: .48 },
  { key: 'superSoftPlush', label: '超柔绒', hint: '柔光细绒 · 亲肤质感', color: 0xf0c9b0, metalness: 0, roughness: .95, family: 'plush', sheen: 1, sheenRoughness: .7 },
  ...
  { key: 'paper', label: '纸质礼盒', hint: '低反射纸感 · 包装预览', color: 0xe7d5bb, metalness: 0, roughness: .86 },
]
...
const selectedPreset = computed(() => presets.find(item => item.key === selected.value) || presets[0])
const isPpcSelected = computed(() => selectedPreset.value.family === 'ppc')
const isPlushSelected = computed(() => selectedPreset.value.family === 'plush')
...
async function applyPreset(key: PresetKey) {
  ...
  try {
    const plushMaterial = preset.family === 'plush'
    const ppcMaterial = preset.family === 'ppc'
    materialMessage.value = plushMaterial ? '正在生成毛绒纤维与柔光效果…' : ppcMaterial ? '正在生成注塑硬塑微表面纹理…' : (preset.textures ? '正在加载真实 PBR 木纹贴图…' : '正在应用参数化 PBR 材质…')
    const plushTextures = plushMaterial ? createPlushTextures(plushColor.value) : undefined
    const ppcTextures = ppcMaterial ? createPpcTextures(ppcColor.value) : undefined
    ...
    root.traverse(child => {
      ...
      const baseColor = plushMaterial ? new THREE.Color(plushColor.value) : ppcMaterial ? new THREE.Color(ppcColor.value) : preset.color
      const material = new THREE.MeshPhysicalMaterial({
        ...
        normalMap: plushMaterial && useTextures ? plushTextures!.normalMap : ppcMaterial && useTextures ? ppcTextures!.normalMap : null,
        roughnessMap: plushMaterial && useTextures ? plushTextures!.roughnessMap : ppcMaterial && useTextures ? ppcTextures!.roughnessMap : null,
        bumpMap: plushMaterial || ppcMaterial ? null : (useTextures ? ('bumpMap' in textures! ? textures!.bumpMap || null : null) : null),
        normalScale: plushMaterial ? new THREE.Vector2(.68, .68) : ppcMaterial ? new THREE.Vector2(.18, .18) : new THREE.Vector2(1, 1),
        bumpScale: useTextures && !plushMaterial && !ppcMaterial ? preset.textures!.bumpScale || .1 : 0,
        sheen: preset.sheen || 0, sheenRoughness: preset.sheenRoughness ?? 1,
        sheenColor: plushMaterial ? new THREE.Color(plushColor.value).lerp(new THREE.Color(0xffffff), .26) : new THREE.Color(0xffffff),
        // 若设计部在 GLB 中导出了顶点色，则在毛绒材质下保留其渐变/花色。
        vertexColors: plushMaterial && hasVertexColors,
      })
      mesh.material = material; generatedMaterials.push(material)
      if (useTextures) texturedMeshCount++
      if (plushMaterial && hasVertexColors) vertexColorMeshCount++
    })
    if (plushMaterial) {
      createPreviewFurShells(plushTextures!)
      materialMessage.value = texturedMeshCount
        ? `已应用「${preset.label}」：纤维颜色、法线、粗糙度、Sheen 柔光与 ${window.innerWidth < 650 ? 5 : 9} 层 Shell Fur 已生效${vertexColorMeshCount ? '，并保留设计部导出的顶点色。' : '。将蓬松长度调高可看到明显的轮廓毛束。'}`
        : '模型没有 UV，毛绒纤维贴图无法铺设；已保留柔光毛绒参数。建议让建模方导出带 UV 的 GLB。'
    } else if (ppcMaterial) {
      materialMessage.value = texturedMeshCount
        ? `已应用「${preset.label}」：低光泽高密度聚合物表面、微橘皮法线与粗糙度变化已生效，可用于精密硬塑摆件的视觉预览。`
        : `该模型没有可用 UV，已应用「${preset.label}」的低光泽硬塑参数；建议使用带 UV 的 GLB 以显示微表面纹理。`
    } else {
...
<template>
  <section class="material-studio">
    ...
    <aside v-if="status==='ready'" class="material-panel">
      ...
      <div v-if="isPpcSelected" class="ppc-controls"><label><span>硬塑基础色</span><input v-model="ppcColor" type="color" @input="applyPreset(selected)" /><b>{{ ppcColor.toUpperCase() }}</b></label><small>低光泽注塑硬塑预览，已包含微橘皮纹理、法线和粗糙度变化。</small></div>
      <div v-if="isPlushSelected" class="plush-controls"><label><span>毛绒底色</span><input v-model="plushColor" type="color" @input="applyPreset(selected)" /><b>{{ plushColor.toUpperCase() }}</b></label><label class="plush-range"><span>纤维密度</span><input v-model.number="plushDensity" type="range" min="20" max="100" @input="applyPreset(selected)" /><b>{{ plushDensity }}%</b></label><label class="plush-range"><span>蓬松长度</span><input v-model.number="plushLength" type="range" min="20" max="100" @input="applyPreset(selected)" /><b>{{ plushLength }}%</b></label><label class="plush-switch"><input v-model="plushPreviewFur" type="checkbox" @change="applyPreset(selected)" /><span>开启边缘蓬松预览</span></label><small>已叠加纤维颜色、法线、粗糙度、柔光和多层 Shell Fur。若模型带顶点色，会保留渐变、斑点等设计细节。</small></div>
      <div class="material-panel-note">{{ materialMessage }}</div>
      <div class="material-actions"><button type="button" class="material-save" :disabled="saving || props.externalSaving" @click="saveVariant">{{ saving || props.externalSaving ? '正在保存…' : '保存到作品库' }}</button><button type="button" class="material-export" :disabled="exporting || props.externalSaving" @click="exportGlb">{{ exporting ? '导出中…' : '下载 GLB ↓' }}</button></div>
    </aside>
  </section>
</template>
...
<style scoped>
.material-preset.abs i,.material-preset.ppc i{background:linear-gradient(135deg,#f0f4f2,#89938f 56%,#dce3df)}
.material-preset.pvc i{background:linear-gradient(135deg,#ffd9c7,#ef805d)}
.material-preset.tangjiao i{background:radial-gradient(circle at 30% 25%,rgba(255,255,255,.62) 0 12%,transparent 13%),linear-gradient(135deg,#ffd9cf,#d8755d)}
.material-preset.softVinyl i{background:linear-gradient(135deg,#e7d7ce,#978074)}
.material-preset.shortPlush i,.material-preset.superSoftPlush i{background:radial-gradient(circle at 28% 30%,#ffe8da 0 5%,transparent 6%),radial-gradient(circle at 65% 55%,#d8795d 0 5%,transparent 6%),radial-gradient(circle at 46% 75%,#f8b496 0 4%,transparent 5%),linear-gradient(135deg,#fac2aa,#a84738)}
.material-preset.paper i{background:linear-gradient(135deg,#fcf1dd,#c7a677)}
</style>
