...
interface WorkflowApplication {
  ...
  flowConfig?: Array<{ index: number; name: string; roles: string[]; mode: string; requiredCount: number }>
  timeline?: Array<{ name: string; status: string; time?: string; operator?: string }>
  logs?: Array<{ id: number; action: string; operator: string; operatorRole: string; comment?: string; createdAt: string }>
}
...
async function act(id: number, action: 'approve' | 'reject') {
  try {
    ...
    await load()
    emit('alert', action === 'approve' ? '已批准申请' : '已驳回申请', 'success')
  } catch (e: any) {
      ...
      <section class="approval-card">
        ...
        <article v-for="item in filteredApplications" :key="item.id" class="approval-item">
          ...
          <div v-if="item.status === 'pending'" class="approve-box">
            ...
            <button class="approve" @click="act(item.id, 'approve')">批准</button>
            <button class="reject" @click="act(item.id, 'reject')">驳回</button>
          </div>
          <div v-if="item.status === 'pending'" class="secondary-actions">
            <button @click="transferVisible[item.id] = !transferVisible[item.id]">转交</button>
            <button v-if="canWithdraw(item)" @click="withdraw(item.id)">撤回</button>
            <span v-if="item.currentHandler">当前处理：{{ item.currentHandler }}</span>
          </div>
          <div v-if="transferVisible[item.id]" class="transfer-box">
