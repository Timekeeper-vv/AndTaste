  ...
  <div class="inventory-page">
    ...
    <section v-if="rows.length" class="asset-grid">
      <article v-for="a in rows" :key="a.id" class="asset-card">
        ...
        <div class="body">
          <div class="product-identity"><span>产品号</span><b :class="{ missing: !a.productNo }">{{ a.productNo || '未关联产品号' }}</b></div>
    ...
    <Teleport to="body">
      <div v-if="active" class="modal" @click.self="close">
        <div class="modal-card">
          <header>
            <div><b>{{ active.title || '库存作品预览' }}</b><span>产品号：{{ active.productNo || '未关联产品号' }} · {{ assetTypeText(active.assetType) }} · 用户ID {{ active.createdBy }} · 已入库</span></div>
