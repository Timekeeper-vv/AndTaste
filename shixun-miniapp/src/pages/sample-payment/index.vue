<template>
  <view class="page">
    ...
    <view v-for="item in requests" :key="requestKey(item)" class="request-card" :class="{ selected: isSelected(item) }" @tap="selectRequest(item)">
      ...
      <text class="note">{{ item.samplePaymentStatus === 'paid' ? '已支付，申请已进入生产' : item.samplePaymentStatus === 'manual_review' ? '待管理员核验收款' : item.samplePaymentStatus === 'pending' ? '支付订单已创建，可继续完成支付' : '请完成打样费用支付' }}</text>
      ...
      <button v-else-if="item.samplePaymentStatus === 'paid'" class="paid" disabled>已支付 · 已进入生产</button>
    </view>
...
function statusLabel(item: any) {
  const paymentStatus = String(item?.samplePaymentStatus || '')
  if (paymentStatus === 'paid') return '已支付'
  if (paymentStatus === 'manual_review') return '待人工核验'
  if (paymentStatus === 'pending') return '支付处理中'
...
async function load() {
  ...
  try {
    ...
    if (quoteId.value) {
      ...
      const paymentStatus = String(item?.samplePaymentStatus || 'unpaid')
      requests.value = item && item.status === 'accepted' && ['unpaid', 'pending', 'manual_review', 'paid'].includes(paymentStatus)
    ...
    const production = (Array.isArray(rows) ? rows : []).filter((item: any) => {
      ...
      return item.requestType === 'sample'
        ...
        && ['unpaid', 'pending', 'manual_review', 'paid'].includes(paymentStatus)
        && (!requestId.value || String(item.id) === requestId.value)
    })
    const quotes = requestId.value ? [] : (data?.quoteRequests || [])
      .filter((item: any) => item.requestType === 'sample' && item.status === 'accepted' && ['unpaid', 'pending', 'manual_review', 'paid'].includes(String(item.samplePaymentStatus || 'unpaid')))
