<template>
  <view class="page">
    <view class="topbar"><view @tap="goBack" class="back">‹</view><view><text class="eyebrow">AI 文创创作</text><text class="top-title">对话式创作</text></view><text class="save-state">{{ saving ? '正在保存' : '已自动保存' }}</text></view>

    <scroll-view class="chat" scroll-y :scroll-into-view="scrollIntoView" scroll-with-animation>
      <view class="progress-card">
        <view class="progress-head"><view><text>本次创作进度</text><text>{{ currentStepLabel }}</text></view><text>{{ currentStepIndex + 1 }}/5</text></view>
        <view class="progress-steps"><view v-for="(item, index) in progressSteps" :key="item" :class="{ active: index <= currentStepIndex, current: index === currentStepIndex }"><text>{{ index + 1 }}</text><text>{{ item }}</text></view></view>
      </view>
      <view class="intro-line"><text>每一步都会自动留存。完成产品图后，可继续修改、生成四视图、3D 原型和生产报价。</text></view>
...
const progressSteps = ['选产品', '说灵感', '定工艺', '生成产品图', '继续落地']
const currentStepIndex = computed(() => {
  if (phase.value === 'mode' || phase.value === 'product') return 0
  if (phase.value === 'inspiration' || phase.value === 'image') return 1
  if (phase.value === 'material') return 2
  if (phase.value === 'style' || phase.value === 'summary') return 3
  return 4
})
const currentStepLabel = computed(() => {
  if (phase.value === 'mode') return '从你的灵感开始'
  if (phase.value === 'product') return '先确定要制作的产品'
  if (phase.value === 'inspiration') return '描述这件产品的灵感'
  if (phase.value === 'image') return '上传一张有权使用的图片'
  if (phase.value === 'material') return '选择材质，或让系统推荐'
  if (phase.value === 'style') return '确定风格和使用场景'
  if (phase.value === 'summary') return '确认方案，准备生成产品图'
  if (phase.value === 'result') return '产品图已完成，可以继续完善'
  if (phase.value === 'multiview') return '四视图已完成，可以生成 3D'
  return isModelTaskSucceeded.value ? '3D 原型已完成' : '正在处理 3D 原型'
})
...
.page{background:#f4f6f4;color:#1d2822}.topbar{border-bottom-color:#dce3de;background:rgba(255,255,255,.97)}.topbar>view:nth-child(2){gap:3rpx}.eyebrow{color:#52765f;font-size:13rpx;letter-spacing:1rpx}.top-title{color:#1e2a24;font-family:"PingFang SC","Microsoft YaHei",sans-serif;font-size:27rpx}.back{color:#294236;font-size:54rpx}.save-state{padding:6rpx 8rpx;border:1rpx solid #d6e2d9;border-radius:5rpx;background:#f1f7f2;color:#54755f;font-size:13rpx}.chat{padding:136rpx 24rpx 28rpx}.progress-card{margin:0 0 14rpx;padding:15rpx;border:1rpx solid #d7e1da;border-radius:8rpx;background:#fff}.progress-head{display:flex;align-items:flex-end;justify-content:space-between;gap:10rpx}.progress-head>view{display:flex;flex-direction:column;gap:4rpx}.progress-head>view text:first-child{color:#22342a;font-size:20rpx;font-weight:850}.progress-head>view text:last-child{color:#728078;font-size:14rpx}.progress-head>text{color:#a44e3d;font-size:16rpx;font-weight:850}.progress-steps{display:flex;justify-content:space-between;gap:3rpx;margin-top:15rpx}.progress-steps>view{position:relative;display:flex;min-width:0;flex:1;align-items:center;flex-direction:column;gap:5rpx;color:#94a098;font-size:12rpx;text-align:center}.progress-steps>view:not(:last-child)::after{content:"";position:absolute;top:13rpx;left:calc(50% + 16rpx);width:calc(100% - 32rpx);height:2rpx;background:#dce5de}.progress-steps>view.active:not(:last-child)::after{background:#6e957c}.progress-steps>view text:first-child{position:relative;z-index:1;display:grid;place-items:center;width:27rpx;height:27rpx;border:1rpx solid #d5ded7;border-radius:50%;background:#fff;font-size:13rpx;font-weight:850}.progress-steps>view.active{color:#52745d}.progress-steps>view.active text:first-child{border-color:#6f927b;background:#e8f1ea;color:#38674c}.progress-steps>view.current text:first-child{border-color:#3f7658;background:#3f7658;color:#fff;box-shadow:0 0 0 5rpx #e5efe8}.intro-line{margin:0 2rpx 17rpx;padding:10rpx 12rpx;border-left-color:#b45542;background:#fffaf5;color:#6f7069;font-size:14rpx}.ai-disclosure{margin-bottom:10rpx}.avatar{border-radius:7rpx;background:#315f48;font-family:"Songti SC","STSong",serif}.bubble{border-color:#dce3de;border-radius:8rpx;background:#fff;box-shadow:none}.bubble text{color:#354039;font-size:19rpx}.user .bubble{border-color:#bed3c4;border-radius:8rpx;background:#e9f2eb}.user .bubble text{color:#315b45}.choice-panel,.input-panel,.summary-panel,.result-panel{border-color:#dce3de;border-radius:8rpx;background:#fff;box-shadow:none}.choice-kicker{display:block;margin-bottom:7rpx;color:#a34d3b;font-size:13rpx;font-weight:850}.choice-title{color:#223129;font-family:"PingFang SC","Microsoft YaHei",sans-serif;font-size:27rpx;line-height:1.3}.choice-note{color:#707d75;font-size:15rpx}.choice-card{border-color:#dce3de;border-radius:7rpx;background:#fff}.choice-card:active,.product-card:active,.material-card:active,.next-card:active,.category-entry:active{border-color:#97b49f;background:#f3f8f4}.choice-mark,.product-mark{border-radius:6rpx;background:#e6f0e8;color:#3d704f}.choice-card:nth-child(2) .choice-mark{background:#f7e7df;color:#a4503d}.choice-card:nth-child(3) .choice-mark{background:#f2eadc;color:#926837}.choice-card view text:first-child{color:#27362e}.choice-card view text:last-child{color:#7b877f}.choice-arrow{color:#b04e3b}.catalog-tools{border-color:#dce3de;border-radius:7rpx;background:#f7f9f7}.catalog-search{border-color:#d8e0da;border-radius:6rpx;background:#fff}.catalog-category{border-radius:5rpx}.catalog-category.active{border-color:#789d84;background:#e8f2ea;color:#3e704f}.category-entry{border-color:#dce3de;border-radius:7rpx}.category-entry-mark{border-radius:6rpx;background:#e7f0e9;color:#3f704f}.catalog-result-title{color:#304237}.product-card{border-color:#dce3de;border-radius:7rpx;background:#fff}.product-card:nth-child(2n) .product-mark{background:#f7e7df;color:#a4503d}.product-card:nth-child(3n) .product-mark{background:#f3eddc;color:#916e37}.product-name{color:#27362e}.product-desc,.product-category-name{color:#7c887f}.product-process{color:#916956}.text-input{border-color:#d7e0da;border-radius:7rpx;background:#fbfcfb;color:#293830}.dark-button,.outline-button{height:78rpx;border-radius:7rpx}.dark-button{background:#bd523f;color:#fff}.dark-button[disabled]{background:#d5dcd7;opacity:1}.outline-button{border-color:#91ad9a;background:#f3f8f4;color:#426c52}.image-picker{height:322rpx;border-color:#8eac99;border-radius:8rpx;background:#f7faf7}.image-picker>view{gap:7rpx;color:#6e8074}.image-picker>view text:first-child{display:grid;place-items:center;width:68rpx;height:68rpx;border-radius:50%;background:#e2eee5;color:#3e7352;font-size:43rpx;line-height:1}.image-picker>view text:last-child{color:#9a887d;font-size:13rpx}.material-card{border-color:#dce3de;border-radius:7rpx;background:#fff}.material-card.active,.recommendation-card{border-color:#91b19b;background:#eef6f0}.recommendation-mark{border-radius:6rpx;background:#3c7454}.pill{border-radius:5rpx;background:#fff}.pill.active{border-color:#6f967d;background:#e8f2ea;color:#386b4d}.summary-card{border-top-color:#dce3de}.summary-card>view{border-bottom-color:#e6ece7}.summary-card text:first-child{color:#819087}.summary-card text:last-child{color:#2b3b31}.result-kicker{color:#a24c3b;font-size:13rpx;letter-spacing:1rpx}.result-image,.result-placeholder{border-radius:8rpx}.result-placeholder{background:linear-gradient(145deg,#dcebe0,#e8dacb);color:#3d6c52}.result-tip{color:#6e7c73}.next-card{border-color:#dce3de;border-radius:7rpx;background:#fff}.next-card>text:first-child{border-radius:6rpx;background:#e8f1e9;color:#3c704f}.next-card>text:last-child{color:#b04e3b}.primary-next{border-color:#9fbeaa;background:#eff7f1}.primary-next>text:first-child{background:#bd523f;color:#fff}.view-card{border-color:#dce3de;border-radius:7rpx}.view-card image,.view-placeholder{background:#edf1ed}.model-success{border-radius:7rpx;background:#eaf3ec}.model-success>text{border-radius:7rpx;background:#3e7454}.model-progress{border-color:#d9e6dc;border-radius:7rpx;background:#f7fbf8}.model-progress>view:first-child{color:#416c51}.model-progress-value{background:#4a7c5d}.model-error{border-radius:7rpx}.loading-bar{border-color:#d5c3ad;border-radius:7rpx;background:#fffaf4;color:#8c5d46}.bottom-actions{border-top-color:#dce3de;background:rgba(255,255,255,.97)}.bottom-actions button{color:#587265}.bottom-actions button:last-child{color:#b04f3c;font-weight:850}.food-direction-note{border-radius:0 7rpx 7rpx 0}.refinement-panel{border-color:#dac7b1;border-radius:7rpx;background:#fff9f1}.first-choice{border-top:3rpx solid #bd523f}
</style>
