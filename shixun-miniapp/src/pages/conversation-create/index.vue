...
async function generateProductImage() {
  ...
  try {
    ...
    let result: any
    if (mode.value === 'image') {
      result = await createImageWithReference({ title: `${selectedProduct.value.name} · 对话创作`, prompt: prompt.value, inputAssetId: referenceAssetId.value, productCategory: selectedProduct.value.name, material: material.value })
    } else {
      result = await createImage({ title: `${selectedProduct.value.name} · 对话创作`, prompt: prompt.value, rawPrompt: inspirationText.value || prompt.value, scene: purpose.value, productType: selectedProduct.value.name, productCategory: selectedProduct.value.name, material: material.value })
    }
    generatedAssetId.value = Number(result?.assetId || result?.id)
    previewUrl.value = imageUrl(result)
    await saveEvent('image', 'image_generated', { productType: selectedProduct.value.name, material: material.value, prompt: prompt.value, generatedAssetId: generatedAssetId.value, previewUrl: previewUrl.value })
