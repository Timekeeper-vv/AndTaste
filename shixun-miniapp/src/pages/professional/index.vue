<template>
  <view class="page">
    ...
    <view class="notice"><text>专业作品审核</text><text>仅接受 ZIP 作品包，最大 100MB。建议包含效果图、尺寸、材质、工艺说明和版权材料。</text></view>
    ...
    <view class="card records-card">
      ...
      <view v-for="record in records" :key="record.id || record.submissionNo" class="record">
        ...
        <view v-if="record.status === 'approved' && record.quotedSampleFeeYuan" class="quote-box">
...
function statusLabel(status?: string) { return status === 'approved' ? '已通过' : status === 'rejected' ? '需修改' : '审核中' }
