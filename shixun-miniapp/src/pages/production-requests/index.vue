    ...
    <view v-else class="request-list">
      ...
      <view v-for="item in requests" :key="item.id" class="request-card">
        <view class="request-head">
          <view class="request-title-wrap"><text class="request-title">{{ item.sampleProductName || item.title || '生产申请' }}</text><text class="request-no">{{ item.requestNo || `申请 #${item.id}` }}</text></view>
...
function sampleRoute(item: any) {
  ...
  return `/pages/sample-lifecycle/index?projectId=${encodeURIComponent(String(item.projectId))}&versionId=${encodeURIComponent(String(item.versionId))}&requestId=${encodeURIComponent(String(item.id))}&title=${encodeURIComponent(item.sampleProductName || item.title || '样品申请')}`
}
...
function goBulkProduction(item: any) {
  ...
  uni.navigateTo({ url: `/pages/production/index?assetId=${encodeURIComponent(String(assetId))}&requestType=bulk&title=${encodeURIComponent(title)}${projectQuery}${versionQuery}` })
}
