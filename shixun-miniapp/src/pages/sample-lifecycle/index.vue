    ...
    <template v-else-if="lifecycle">
      <view class="summary-card">
        <view class="summary-head">
          <view>
            <text class="product-no">产品号：{{ productNo || lifecycle.productNo || '未关联产品号' }}</text>
...
async function load() {
  ...
  try {
    ...
    if (!productNo.value && nextLifecycle.productNo) productNo.value = String(nextLifecycle.productNo)
...
onLoad((query: any) => {
  ...
  productNo.value = String(query?.productNo || '')
