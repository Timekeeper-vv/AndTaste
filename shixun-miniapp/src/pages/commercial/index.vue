  ...
  <view class="page">
    ...
    <view v-if="channelPickerVisible" class="channel-mask" @tap="closeChannelPicker">
      <view class="channel-modal" @tap.stop>
        <view class="channel-modal-head"><view><text>选择售卖渠道</text><text>先选择省份，再选择博物馆或景区</text></view><text class="channel-close" @tap="closeChannelPicker">×</text></view>
        ...
        <text class="channel-filter-label">1. 选择省份</text>
        ...
        <text class="channel-filter-label">2. 选择渠道类型</text>
        ...
        <text class="channel-result-note">{{ channelDirectory.total }} 条{{ channelProvince ? `${channelProvince}的` : '' }}博物馆/景区 · 候选渠道须经运营、版权和授权核验</text>
...
const boundProduct = ref<CommercialProduct | null>(null)
const boundProductKey = ref('')
const boundProductName = ref('')
const boundMaterial = ref('')
...
onLoad(async (query: any) => {
  ...
  boundProductKey.value = String(query?.productKey || '')
...
function bindProductFromCreation() {
  if (!boundProductKey.value && !boundProductName.value) return
  const matched = products.value.find((product) =>
    (boundProductKey.value && (product.optionKey === boundProductKey.value || product.templateCode === boundProductKey.value))
    || (boundProductName.value && product.productName === boundProductName.value),
  )
  if (!matched) {
    uni.showToast({ title: '本次产品暂未开放商品化模板，请联系运营', icon: 'none' })
    return
  }
...
function channelTypeLabel(value?: string) { return ({ museum: '博物馆', scenic_spot: '景区' } as Record<string, string>)[String(value || '')] || '渠道' }
...
async function loadChannels(reset = true) {
  ...
  try {
    ...
    const response = await getCommercialChannelDirectory({ keyword: channelKeyword.value.trim(), type: channelType.value, province: channelProvince.value, page, size: 30 })
...
function setChannelType(value: string) { channelType.value = value; void loadChannels(true) }
...
function orderPriority(item: any) {
  ...
  if (awaitingSamplePayment) return 0
  if (item.status === 'quoted') return 1
  if (item.status === 'accepted') return 2
  if (['new', 'processing', 'pending_review', 'need_materials'].includes(item.status)) return 3
  return 4
}
