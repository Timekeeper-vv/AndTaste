<template>
  <view class="page">
    ...
    <view class="mode-grid">
      <view class="mode-choice amateur" :class="{active: creatorMode==='amateur'}" @tap="creatorMode='amateur'"><text class="mode-icon">聊</text><view><text class="name">业余创作</text><text class="intro">对话式引导、模板选品、快速生图，适合第一次创作</text><text class="mode-features">选产品 · 说灵感 · 自动推荐 · 申请打样</text></view><text class="check">{{ creatorMode==='amateur' ? '✓' : '' }}</text></view>
      <view class="mode-choice professional" :class="{active: creatorMode==='professional'}" @tap="creatorMode='professional'"><text class="mode-icon">专</text><view><text class="name">专业创作</text><text class="intro">上传专业作品包，补充规格工艺，进入评审与生产流程</text><text class="mode-features">作品包 · 四视图/3D · 版权材料 · 渠道投稿</text></view><text class="check">{{ creatorMode==='professional' ? '✓' : '' }}</text></view>
    </view>
    ...
    <button class="enter" :disabled="!canEnter" @tap="enter">{{ creatorMode === 'professional' ? '进入专业创作' : '进入业余创作' }}</button>
  </view>
</template>
...
function enter(){if(!canEnter.value)return;uni.setStorageSync('creation_context',{creatorMode:creatorMode.value,purpose:purpose.value,museum:museum.value});uni.reLaunch({url:creatorMode.value==='professional'?'/pages/professional/index':'/pages/home/index'})}
onMounted(async()=>{if(!requireSession())return;const saved=uni.getStorageSync('creation_context')||{};if(saved.creatorMode==='professional'||saved.creatorMode==='amateur')creatorMode.value=saved.creatorMode;if(saved.purpose==='personal'||saved.purpose==='museum_sale')purpose.value=saved.purpose;if(saved.museum){museum.value=saved.museum;province.value=saved.museum.province||''}try{museums.value=await getMuseums()}catch(e:any){uni.showToast({title:e.message||'博物馆目录加载失败',icon:'none'})}})
</script>
