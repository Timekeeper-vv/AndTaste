  ...
  <view class="page">
    ...
    <view class="hero-card">
      <view class="hero-top">
        ...
        <view class="purpose-pill" @tap="go('/pages/purpose/index')"><text>{{ creatorModeLabel }} · {{ purposeBadge }}</text><text>切换</text></view>
      </view>
    ...
    <view v-if="isProfessional" class="professional-promo" @tap="go('/pages/professional/index')"><view class="professional-mark">专</view><view><text>专业创作工作台</text><text>上传作品包，进入 AI 评审、版权核验与生产对接</text></view><text>进入 ›</text></view>

    <view class="conversation-promo" @tap="go('/pages/conversation-create/index')">
      <view class="conversation-promo-mark">聊</view>
      <view class="conversation-promo-main">
        <text class="conversation-promo-kicker">推荐入口 · CONVERSATIONAL STUDIO</text>
        <text class="conversation-promo-title">开始对话式创作</text>
        <text class="conversation-promo-desc">像聊天一样回答几个问题，把灵感一步步做成可生成、可建模的文创产品</text>
        <view class="conversation-steps"><text>01 选产品</text><text>02 说灵感</text><text>03 出成图</text></view>
      </view>
      <view class="conversation-promo-action"><text>立即开始</text><text>›</text></view>
    </view>

    <view v-if="featuredReward" class="reward-board">
      <view class="reward-board-head"><view><text class="eyebrow">CREATOR REWARDS</text><text class="reward-board-title">完成创作，领取额外积分</text></view><text class="reward-board-link" @tap="go('/pages/works/index')">任务记录 ›</text></view>
      <view class="reward-feature" @tap="handleRewardAction(featuredReward)">
        <view class="reward-feature-copy"><text class="reward-kicker">新手创作礼</text><text class="reward-feature-title">{{ featuredReward.title }}</text><text class="reward-feature-desc">{{ featuredReward.description }}</text><text class="reward-feature-status">{{ rewardStatusLabel(featuredReward) }}</text></view>
        <view class="reward-feature-points"><text>+{{ featuredReward.rewardAmount }}</text><text>积分</text></view>
        <text class="reward-feature-action">{{ rewardActionLabel(featuredReward) }} ›</text>
      </view>
      <view class="reward-mission-list"><view v-for="mission in otherRewards" :key="mission.key" class="reward-mission" @tap="handleRewardAction(mission)"><text class="reward-mission-mark">{{ missionMark(mission.key) }}</text><view><text>{{ mission.title }}</text><text>+{{ mission.rewardAmount }} 积分 · {{ rewardStatusLabel(mission) }}</text></view><text class="reward-mission-action">{{ rewardActionLabel(mission) }} ›</text></view></view>
    </view>
    ...
    <view class="native-service-section">
      <view class="section-head compact"><view><text class="eyebrow">NATIVE SERVICE HUB</text><text class="title">创作之外，也都在这里。</text></view><text>原生服务</text></view>
      <view class="native-service-grid">
        <view v-for="service in nativeServices" :key="service.title" class="native-service-card" :class="service.tone" @tap="service.action">
          <text class="native-service-mark">{{ service.mark }}</text>
          <view><text>{{ service.title }}</text><text>{{ service.desc }}</text></view>
          <text class="native-service-arrow">›</text>
        </view>
      </view>
    </view>
...
import { claimRewardMission, getCredits, getRewardOverview } from '../../api/creative'
...
const otherRewards = computed(() => featuredReward.value ? rewardMissions.value.filter(item => item.key !== featuredReward.value.key) : [])
...
const nativeServices = [
  { mark: '集', title: '文创商城', desc: '浏览已审核的文化作品与衍生品', tone: 'market', action: () => go('/pages/market/index') },
  { mark: '单', title: '我的订单', desc: '查看商城订单和处理状态', tone: 'orders', action: () => go('/pages/orders/index') },
  { mark: '审', title: '审核与打样', desc: '跟进作品审核、打样和生产申请', tone: 'review', action: () => go('/pages/works/index') },
  { mark: '问', title: '在线服务', desc: '咨询创作、支付、版权与合作', tone: 'support', action: () => go('/pages/support/index?tab=chat') },
]
...
function missionMark(key: string) {
  return key === 'first_model_success' ? '3D' : key === 'first_review_submit' ? '审' : key === 'first_approved_work' ? '过' : key === 'first_sample_request' ? '样' : '图'
}
function rewardStatusLabel(mission: any) {
  return mission?.status === 'claimed' ? '已领取' : mission?.status === 'claimable' ? '可领取' : '待完成'
}
function rewardActionLabel(mission: any) {
  return mission?.status === 'claimed' ? '已完成' : mission?.status === 'claimable' ? '领取积分' : mission?.key === 'first_image_success' ? '去生图' : mission?.key === 'first_model_success' ? '去建模' : mission?.key === 'first_review_submit' ? '去提交审核' : mission?.key === 'first_approved_work' ? '查看作品' : '去申请打样'
}
async function handleRewardAction(mission: any) {
  ...
  if (mission.key === 'first_image_success') beginCreation('concept')
  else if (mission.key === 'first_model_success') beginCreation('prototype')
  else go('/pages/works/index')
}
async function loadRewards() {
  try { rewardOverview.value = await getRewardOverview() } catch { rewardOverview.value = { missions: [] } }
}
async function refreshHome() {
  user.value = getSession()?.user
  context.value = uni.getStorageSync('creation_context')
  try { credits.value = Number((await getCredits()).balance) || 0 } catch { /* 余额加载失败不阻塞本地入口 */ }
  await loadRewards()
}
...
.conversation-promo{display:grid;position:relative;z-index:1;grid-template-columns:64rpx minmax(0,1fr) auto;align-items:center;gap:14rpx;margin-top:18rpx;padding:20rpx 18rpx;border:1rpx solid #a9c3b0;border-radius:23rpx;background:linear-gradient(135deg,#dfeee2 0%,#f7f3e9 58%,#f9e8db 100%);box-shadow:0 14rpx 30rpx rgba(79,99,81,.15)}.conversation-promo::after{content:"";position:absolute;right:112rpx;bottom:-16rpx;width:130rpx;height:35rpx;border-radius:50%;background:rgba(102,130,113,.14);filter:blur(13rpx);pointer-events:none}.conversation-promo-mark{position:relative;z-index:1;display:grid;place-items:center;width:62rpx;height:62rpx;border:2rpx solid rgba(255,255,255,.76);border-radius:19rpx;background:#5d7f6c;color:#fff;font-family:"Songti SC","STSong",serif;font-size:31rpx;font-weight:800;box-shadow:0 7rpx 13rpx rgba(56,82,66,.2)}.conversation-promo-main{position:relative;z-index:1;display:flex;min-width:0;flex-direction:column;gap:5rpx}.conversation-promo-kicker{color:#5f806d;font-size:13rpx;font-weight:900;letter-spacing:1.4rpx}.conversation-promo-title{color:#35463a;font-family:"Songti SC","STSong",serif;font-size:31rpx;font-weight:800}.conversation-promo-desc{color:#706d63;font-size:15rpx;line-height:1.45}.conversation-steps{display:flex;flex-wrap:wrap;gap:6rpx;margin-top:4rpx}.conversation-steps text{padding:5rpx 8rpx;border:1rpx solid rgba(121,151,132,.3);border-radius:999rpx;background:rgba(255,255,255,.44);color:#668170;font-size:13rpx;font-weight:800}.conversation-promo-action{position:relative;z-index:1;display:flex;align-items:flex-end;flex-direction:column;gap:3rpx;color:#a15f49;font-weight:900;white-space:nowrap}.conversation-promo-action text:first-child{padding:6rpx 8rpx;border:1rpx solid rgba(161,95,73,.22);border-radius:9rpx;background:rgba(255,250,244,.65);font-size:14rpx}.conversation-promo-action text:last-child{font-size:35rpx;line-height:1}.conversation-promo:active{opacity:.82;transform:translateY(1rpx)}
.professional-promo{display:flex;align-items:center;gap:15rpx;margin-top:22rpx;padding:20rpx;border:1rpx solid #d9e7dc;border-radius:20rpx;background:#eff6f0;color:#547363;box-shadow:0 10rpx 22rpx rgba(63,82,69,.07)}.professional-mark{display:grid;place-items:center;flex:none;width:54rpx;height:54rpx;border-radius:15rpx;background:#dceade;color:#4d735f;font-family:"Songti SC","STSong",serif;font-size:27rpx;font-weight:800}.professional-promo view:nth-child(2){display:flex;min-width:0;flex:1;flex-direction:column}.professional-promo view:nth-child(2) text:first-child{font-size:25rpx;font-weight:800}.professional-promo view:nth-child(2) text:last-child{margin-top:5rpx;color:#778a7e;font-size:18rpx;line-height:1.45}.professional-promo>text:last-child{font-size:21rpx;color:#658474}
</style>
<style scoped lang="scss">
.reward-board{position:relative;z-index:1;margin-top:18rpx;padding:22rpx;border:1rpx solid #e6d7c5;border-radius:26rpx;background:linear-gradient(145deg,#fffaf1,#f4eee4 62%,#e7f0e8);box-shadow:0 14rpx 30rpx rgba(78,61,42,.08)}.reward-board-head{display:flex;align-items:flex-end;justify-content:space-between;gap:12rpx}.reward-board-head>view{display:flex;flex-direction:column;gap:6rpx}.reward-board-title{color:#3e3932;font-family:"Songti SC","STSong",serif;font-size:31rpx;font-weight:800}.reward-board-link{padding:6rpx 8rpx;border:1rpx solid #e2d8ca;border-radius:9rpx;background:#fffdf8;color:#8a705f;font-size:15rpx}.reward-feature{position:relative;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:11rpx;min-height:176rpx;margin-top:16rpx;padding:18rpx;border:1rpx solid #e2c7a6;border-radius:20rpx;background:linear-gradient(135deg,#fff9eb,#f7eadb);box-shadow:0 8rpx 18rpx rgba(132,91,54,.08)}.reward-feature-copy{display:flex;min-width:0;flex-direction:column}.reward-kicker{color:#ad694b;font-size:15rpx;font-weight:900;letter-spacing:1.5rpx}.reward-feature-title{margin-top:8rpx;overflow:hidden;color:#493b32;font-family:"Songti SC","STSong",serif;font-size:27rpx;font-weight:800;text-overflow:ellipsis;white-space:nowrap}.reward-feature-desc{display:-webkit-box;overflow:hidden;margin-top:7rpx;color:#816f62;font-size:16rpx;line-height:1.5;-webkit-box-orient:vertical;-webkit-line-clamp:2}.reward-feature-status{margin-top:auto;color:#997056;font-size:15rpx;font-weight:800}.reward-feature-points{display:flex;flex-direction:column;align-items:center;justify-content:center;min-width:104rpx;min-height:104rpx;border:1rpx solid #d6a97e;border-radius:50%;background:#fff5df;color:#a95636;transform:rotate(4deg)}.reward-feature-points text:first-child{font-family:"Songti SC","STSong",serif;font-size:40rpx;font-weight:900;line-height:1}.reward-feature-points text:last-child{margin-top:5rpx;font-size:16rpx;font-weight:900}.reward-feature-action{position:absolute;right:18rpx;bottom:13rpx;color:#ae6248;font-size:15rpx;font-weight:900}.reward-mission-list{display:grid;gap:9rpx;margin-top:11rpx}.reward-mission{display:grid;grid-template-columns:42rpx minmax(0,1fr) auto;align-items:center;gap:9rpx;min-height:70rpx;padding:10rpx 11rpx;border:1rpx solid rgba(215,205,191,.8);border-radius:15rpx;background:rgba(255,253,248,.82)}.reward-mission-mark{display:grid;place-items:center;width:40rpx;height:40rpx;border-radius:11rpx;background:#e8f0e8;color:#557363;font-family:"Songti SC","STSong",serif;font-size:21rpx;font-weight:900}.reward-mission view{display:flex;min-width:0;flex-direction:column;gap:4rpx}.reward-mission view text:first-child{overflow:hidden;color:#4a4239;font-size:19rpx;font-weight:850;text-overflow:ellipsis;white-space:nowrap}.reward-mission view text:last-child{color:#8d7d70;font-size:14rpx}.reward-mission-action{color:#a3634e;font-size:14rpx;font-weight:900;white-space:nowrap}.reward-mission:active,.reward-feature:active{opacity:.82}
</style>
<style scoped lang="scss">
.native-service-section{margin-top:24rpx;padding:24rpx;border:1rpx solid rgba(133,120,102,.14);border-radius:29rpx;background:rgba(255,253,249,.74);box-shadow:0 12rpx 30rpx rgba(70,56,39,.045)}.native-service-grid{display:grid;grid-template-columns:1fr 1fr;gap:11rpx;margin-top:17rpx}.native-service-card{display:grid;grid-template-columns:42rpx minmax(0,1fr) auto;gap:8rpx;align-items:center;min-height:100rpx;padding:13rpx;border:1rpx solid #e5ddd2;border-radius:18rpx;background:#fffefa;box-sizing:border-box}.native-service-card:active{background:#f5f1e9}.native-service-mark{display:grid;place-items:center;width:40rpx;height:40rpx;border-radius:12rpx;font-family:"Songti SC","STSong",serif;font-size:24rpx;font-weight:800}.native-service-card view{display:flex;min-width:0;flex-direction:column}.native-service-card view text:first-child{overflow:hidden;color:#4b463e;font-size:19rpx;font-weight:850;text-overflow:ellipsis;white-space:nowrap}.native-service-card view text:last-child{display:-webkit-box;overflow:hidden;margin-top:4rpx;color:#92867b;font-size:13rpx;line-height:1.35;-webkit-box-orient:vertical;-webkit-line-clamp:2}.native-service-arrow{color:#9f9081;font-size:30rpx;line-height:1}.native-service-card.market .native-service-mark{color:#5a7a6c;background:#e7f0e8}.native-service-card.orders .native-service-mark{color:#a06149;background:#f8e9df}.native-service-card.review .native-service-mark{color:#6c668b;background:#eeeaf5}.native-service-card.support .native-service-mark{color:#47806d;background:#e3f2eb}
</style>
