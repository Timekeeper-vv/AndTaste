    ...
    <view class="atelier-card">
      ...
      <view class="studio-canvas">
        ...
        <view class="preview-panel">
          ...
          <view class="preview-stage" :style="{ '--tone': selectedPattern.tone }"><view class="preview-halo" /><view class="preview-product"><text>{{ selectedPattern.mark }}</text></view><view class="preview-label"><text>{{ selectedPattern.name }}</text><text>{{ atelier.material }} · {{ materialFinishText }}</text></view></view>
      ...
      <view class="finish-block">
        ...
        <view class="material-grid"><view v-for="material in materials" :key="material.name" class="material-choice" :class="{ active: atelier.material === material.name }" @tap="chooseMaterial(material)"><text class="material-swatch" :class="material.key" /><view><text>{{ material.name }}</text><text>{{ material.note }}</text></view><text>✓</text></view></view>
...
const materials = [
  { key: 'glaze', name: '陶瓷釉面', modelMaterial: '陶瓷釉面', note: '温润通透，适合器物与礼盒摆件' },
  { key: 'bronze', name: '青铜金属', modelMaterial: '金属质感', note: '克制哑光，适合浮雕与纪念章感' },
  { key: 'wood', name: '木质温润', modelMaterial: '木质温润', note: '自然年轮，适合文具与随身小物' },
  { key: 'jade', name: '玉感树脂', modelMaterial: '树脂潮玩', note: '细腻半透，适合潮玩与桌面陈列' },
]
...
function chooseMaterial(material: typeof materials[number]) { atelier.material = material.name; atelier.modelMaterial = material.modelMaterial }
function changeFinish(key: FinishKey, event: any) { atelier[key] = Number(event.detail.value) || 0 }
function buildPrompt() {
  const direction = `材质表现：${atelier.material}，釉面光泽 ${atelier.glaze}%，肌理颗粒 ${atelier.texture}%，浮雕层次 ${atelier.relief}%`
  const source = atelier.prompt.replace(/(?:，|,)?材质表现：[^。；;]*(?:[。；;]|$)/g, '').replace(/[，,。；;\s]+$/, '')
...
function beginCreation(mode = atelierMode.value) {
  uni.setStorageSync('miniapp_atelier_draft', { mode: creationMode(mode), title: `${selectedPattern.value.name} · ${atelier.material}`, prompt: buildPrompt(), pattern: selectedPattern.value, material: atelier.material, modelMaterial: atelier.modelMaterial, glaze: atelier.glaze, texture: atelier.texture, relief: atelier.relief })
  go(`/pages/create/index?mode=${creationMode(mode)}`)
}
function openImageTo3d() {
  uni.setStorageSync('miniapp_atelier_draft', { mode: 'image3d', title: `${selectedPattern.value.name} · ${atelier.material}`, prompt: buildPrompt(), pattern: selectedPattern.value, material: atelier.material, modelMaterial: atelier.modelMaterial, glaze: atelier.glaze, texture: atelier.texture, relief: atelier.relief })
