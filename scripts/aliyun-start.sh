...
VOLCENGINE_ARK_API_KEY="${VOLCENGINE_ARK_API_KEY:-}"; VOLCENGINE_ARK_IMAGES_BASE_URL="${VOLCENGINE_ARK_IMAGES_BASE_URL:-https://ark.cn-beijing.volces.com/api/v3/images/generations}"; VOLCENGINE_ARK_SEEDREAM_IMAGE_MODEL="${VOLCENGINE_ARK_SEEDREAM_IMAGE_MODEL:-doubao-seedream-5-0-pro-260628}"; VOLCENGINE_ARK_SEEDREAM_MULTIVIEW_MODEL="${VOLCENGINE_ARK_SEEDREAM_MULTIVIEW_MODEL:-doubao-seedream-5-0-260128}"
...
PAYMENT_WECHAT_VIRTUAL_ENABLED="${PAYMENT_WECHAT_VIRTUAL_ENABLED:-false}"
PAYMENT_WECHAT_VIRTUAL_OFFER_ID="${PAYMENT_WECHAT_VIRTUAL_OFFER_ID:-}"
PAYMENT_WECHAT_VIRTUAL_ENV="${PAYMENT_WECHAT_VIRTUAL_ENV:-0}"
PAYMENT_WECHAT_VIRTUAL_PRODUCTION_APP_KEY="${PAYMENT_WECHAT_VIRTUAL_PRODUCTION_APP_KEY:-}"
PAYMENT_WECHAT_VIRTUAL_SANDBOX_APP_KEY="${PAYMENT_WECHAT_VIRTUAL_SANDBOX_APP_KEY:-}"
PAYMENT_WECHAT_VIRTUAL_SESSION_ENCRYPTION_KEY="${PAYMENT_WECHAT_VIRTUAL_SESSION_ENCRYPTION_KEY:-}"
JAR_FILE=""
...
volcengine.ark.images.base-url=$VOLCENGINE_ARK_IMAGES_BASE_URL
volcengine.ark.seedream.image.model=$VOLCENGINE_ARK_SEEDREAM_IMAGE_MODEL
volcengine.ark.seedream.multiview.model=$VOLCENGINE_ARK_SEEDREAM_MULTIVIEW_MODEL
...
payment.wechat.virtual.enabled=$PAYMENT_WECHAT_VIRTUAL_ENABLED
payment.wechat.virtual.offer-id=$PAYMENT_WECHAT_VIRTUAL_OFFER_ID
payment.wechat.virtual.env=$PAYMENT_WECHAT_VIRTUAL_ENV
payment.wechat.virtual.production-app-key=$PAYMENT_WECHAT_VIRTUAL_PRODUCTION_APP_KEY
payment.wechat.virtual.sandbox-app-key=$PAYMENT_WECHAT_VIRTUAL_SANDBOX_APP_KEY
payment.wechat.virtual.session-encryption-key=$PAYMENT_WECHAT_VIRTUAL_SESSION_ENCRYPTION_KEY
...
validate_runtime_config(){
  ...
  if [ "$PAYMENT_WECHAT_VIRTUAL_ENABLED" = "true" ]; then
    [ "$PAYMENT_WECHAT_APP_ID" = "wxd1ba9e6e01d0e3db" ] || die "PAYMENT_WECHAT_APP_ID 必须与已发布小程序 AppID wxd1ba9e6e01d0e3db 一致"
    [ -n "$PAYMENT_WECHAT_MINI_APP_SECRET" ] || die "启用微信虚拟支付时必须填写 PAYMENT_WECHAT_MINI_APP_SECRET"
    [ -n "$PAYMENT_WECHAT_VIRTUAL_OFFER_ID" ] || die "PAYMENT_WECHAT_VIRTUAL_OFFER_ID 不能为空"
    [[ "$PAYMENT_WECHAT_VIRTUAL_ENV" =~ ^[01]$ ]] || die "PAYMENT_WECHAT_VIRTUAL_ENV 只能是 0（正式）或 1（沙箱）"
    if [ "$PAYMENT_WECHAT_VIRTUAL_ENV" = "0" ]; then
      [ -n "$PAYMENT_WECHAT_VIRTUAL_PRODUCTION_APP_KEY" ] || die "正式虚拟支付必须填写 PAYMENT_WECHAT_VIRTUAL_PRODUCTION_APP_KEY"
    else
      [ -n "$PAYMENT_WECHAT_VIRTUAL_SANDBOX_APP_KEY" ] || die "沙箱虚拟支付必须填写 PAYMENT_WECHAT_VIRTUAL_SANDBOX_APP_KEY"
    fi
