#!/usr/bin/env python3
"""Generate idempotent SQL import for supply-chain sample work-order Excel files.
...
FIELD_ORDER = [
    ...
    "approval_node", "application_department", "applicant", "project_name", "product_name", "order_type",
    "product_type", "product_sub_type", "product_estimate", "product_estimate_currency", "sample_quantity_text",
    "spec_flavor", "sample_fee_yuan", "detail_remark", "detail_project_name", "linked_project_flow",
    "attachment_summary", "source_id", "work_order_status", "start_date", "estimated_complete_date",
    "actual_complete_date", "owner", "factory", "sample_cost_yuan", "sample_file_provided_date",
    "row_checksum", "raw_json",
]
COL_TO_FIELD = dict(zip(COLS, FIELD_ORDER[4:38]))
...
def excel_serial_to_datetime(value: str, as_date: bool = False) -> str | None:
    ...
    # Round to seconds, avoiding float artifacts from Excel fractions.
...
def normalize_money(value: str) -> str | None:
    ...
    if not s:
        return None
    # Keep only clean numeric values in decimal columns; mixed text remains available in raw_json.
    ...
    for offset, row in enumerate(rows[1:], start=2):
        ...
        for col in COLS:
            ...
            if col in DATE_TIME_COLS:
                ...
                display = raw.strip() if isinstance(raw, str) else raw
        ...
        for col in COLS:
            ...
            if col in DATE_TIME_COLS:
                ...
                rec[field] = raw.strip() if isinstance(raw, str) else raw
        records.append(rec)
...
def write_sql(records: list[dict], output: Path):
    ...
    lines = [
        "-- Auto-generated by scripts/generate_sample_work_order_sql.py",
        "-- Idempotent import: one Excel detail row becomes one supply_chain_sample_work_order row.",
        "",
    ]
    if records:
    ...
    for rec in records:
        ...
        lines.append(f"INSERT IGNORE INTO supply_chain_sample_work_order ({columns}) VALUES ({values});")
    output.write_text("\n".join(lines) + "\n", encoding="utf-8")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("xlsx", nargs="?", default="/Users/mac/Documents/供应链工单明细_2026打样申请_打样申请 (1).xlsx")
    parser.add_argument("--output", default="shixun/src/main/resources/sample_work_order_import.sql")
    args = parser.parse_args()
    xlsx = Path(args.xlsx).expanduser().resolve()
    _, _, records = build_records(xlsx)
